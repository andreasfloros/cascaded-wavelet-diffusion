...
class CascadedWaveletDiffuser(th.nn.Module):
    ...
    def calculate_stats(self, dataloader: DataLoader) -> None:
        ...
        for wd in self.wds:
            ...
            wd.transform.min.copy_(th.inf)
            wd.transform.max.copy_(-th.inf)
        ...
        for inpt, _ in dataloader:
            ...
            for wd in self.wds:
                ...
                wd.transform.details_min.copy_(th.min(wd.transform.min, details.min()))
                wd.transform.details_max.copy_(th.max(wd.transform.max, details.max()))
